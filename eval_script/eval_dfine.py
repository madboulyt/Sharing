...
#### endpoint run
def run_object_detection_endpoint(
    img_paths: list,
    ...
    for i, p in enumerate(img_paths):
        # ---- Decode image + force RGB ----
        with Image.open(p) as img:
            img = img.convert("RGB")  #TODO: does it matter if we send base64 then convert to RGB? or it should be converted before encoding? (I think it should be converted before encoding to ensure consistent format)
            buf = BytesIO()
            img.save(buf, format="PNG")#"JPEG", quality=95)  
    ...
    payload = {"batch": batch}
    response = requests.post(object_detection_url, json=payload, timeout=timeout)
...
def evaluate_at_thr(
    ...
    object_detection_url: str,
    batch_size: int = 8,
    conf_thr_for_eyeball: float = 0.3
):
    f1_metric = F1Score()
    ...
    for i in tqdm(range(0, len(img_paths), batch_size), desc="Object Detection inference"):
        img_batch = img_paths[i:i + batch_size]
        gt_batch = gt_dets[i:i + batch_size]

        preds = run_object_detection_endpoint(img_batch, object_detection_url)

        for gt, pr in zip(gt_batch, preds):
            all_gt.append(gt)
            all_pr.append(pr)

            if conf_thr_for_eyeball is not None and len(pr):
                pr_eval = pr[pr.confidence >= conf_thr_for_eyeball]
            else:
                pr_eval = pr

            f1_metric.update(pr_eval, gt)
            mAP_metric.update(pr_eval, gt)
            mAR_metric.update(pr_eval, gt)
            precision_metric.update(pr_eval, gt)
            recall_metric.update(pr_eval, gt)
...
if __name__ == "__main__":
    ...
    # COCO GT
    parser.add_argument("--val_coco_json", type=str, required=False)
    parser.add_argument("--val_coco_imgs_dir", type=str, required=False)
    parser.add_argument("--test_coco_json", type=str, required=True)
    parser.add_argument("--test_coco_imgs_dir", type=str, required=True)

    # Eval controls
    parser.add_argument("--object_detection_url", type=str,
    default="http://localhost:8100/layout-detection")

    parser.add_argument("--mode", choices=["val_test", "test_only"],
        default="val_test")

    parser.add_argument("--best_conf", type=float, default=None)
    ...
    if args.use_coco_categories:
        classes_ = read_coco_categories(args.test_coco_json)
    ...
    if classes_ is None:
        ...
        with open(args.test_coco_json, "r", encoding="utf-8") as f:
            data_coco = json.load(f)
    ...
    #if  args.mode in {"val_test", "test_only"}:
    #if args.best_conf is None:
    #    raise ValueError("--best_conf must be provided in test_only mode")
    if args.mode == "test_only":
        best_conf = args.best_conf
    else:
        best_conf = best_conf
    ...
    f1_test, mAP_test, mAR_test, p_test, r_test, test_all_gt, test_all_pred = (
        evaluate_at_thr(
            test_paths,
            test_gt,
            args.object_detection_url,
    ...
    with open(result_path, "w", encoding="utf-8") as f:
        json.dump({
           ...
           "per_class_best_conf": per_class if args.mode == "val_test" else None,
...
python eval_dfine.py \
  --test_coco_json "/ephemeral/home/data/TalkToDocs-Ingest/data/mozn_e2e_eval_data/task_248_dataset_2026_01_26_09_55_18_cvat for images 1.1/layout_gt.json" \
  --test_coco_imgs_dir "/ephemeral/home/data/TalkToDocs-Ingest/data/mozn_e2e_eval_data/task_248_dataset_2026_01_26_09_55_18_cvat for images 1.1/images" \
  --object_detection_url http://localhost:8100/layout-detection \
  --mode test_only \
  --best_conf 0.662 \
  --eval_output /ephemeral/home/abuzayed/e2e_evaluation_V2/eval_layout \
  --batch_size 16
"""
...
Val-test mode
python eval_dfine.py \
  --val_coco_json /ephemeral/home/data/TalkToDocs-Ingest/data/layout/training_data/dataset6_batch0_2_3_4_5_6_7_8_9_10/val/annotations.coco.json \
  --val_coco_imgs_dir /ephemeral/home/data/TalkToDocs-Ingest/data/layout/training_data/dataset6_batch0_2_3_4_5_6_7_8_9_10/val/images \
  --test_coco_json /ephemeral/home/data/TalkToDocs-Ingest/data/layout/training_data/dataset6_batch0_2_3_4_5_6_7_8_9_10/test/annotations.coco.json \
  --test_coco_imgs_dir /ephemeral/home/data/TalkToDocs-Ingest/data/layout/training_data/dataset6_batch0_2_3_4_5_6_7_8_9_10/test/images \
  --object_detection_url http://localhost:8100/layout-detection \
  --mode val_test \
  --eval_output /ephemeral/home/abuzayed/e2e_evaluation_V2/test_images/eval_output \
  --batch_size 16 \
  --iou_for_curves 0.5 \
  --beta 1.0 \
  --per_class 1
...
LINE EVAL
python eval_dfine.py \
  --val_coco_imgs_dir /ephemeral/home/data/TalkToDocs-Ingest/data/line/cvat_exported_data_nov2025/merged_dataset/val/images \
  --val_coco_json /ephemeral/home/data/TalkToDocs-Ingest/data/line/cvat_exported_data_nov2025/merged_dataset/val/annotations.json \
  --test_coco_imgs_dir /ephemeral/home/data/TalkToDocs-Ingest/data/line/cvat_exported_data_nov2025/merged_dataset/test/images \
  --test_coco_json /ephemeral/home/data/TalkToDocs-Ingest/data/line/cvat_exported_data_nov2025/merged_dataset/test/annotations.json \
  --object_detection_url http://localhost:8101/line-detection \
  --mode val_test \
  --eval_output /ephemeral/home/abuzayed/e2e_evaluation_V2/test_images/eval_output_line \
  --batch_size 64 \
  --iou_for_curves 0.50 \
  --beta 1.0 \
  --per_class 1
  ...
  --mode test_only \
  --best_conf 0.687 \
  --eval_output /ephemeral/home/abuzayed/e2e_evaluation_V2/test_images/eval_output_line_test \
  --batch_size 64 \
  --iou_for_curves 0.50 \
  --beta 1.0 \
  --per_class 1
